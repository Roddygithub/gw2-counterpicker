...
def convert_parsed_log_to_players_data(parsed_log) -> dict:
    ...
    for player in parsed_log.players:
        ...
        player_data = {
            ...
            'downs': downs,
            'cc_out': getattr(player, 'cc_out', 0) or 0,
            'cc_in': 0,
            'boon_strips': getattr(player, 'boon_strips', 0) or 0,
            'boon_strip_in': 0,
            'strips_per_sec': round((getattr(player, 'boon_strips', 0) or 0) / duration_sec, 2),
            # Support stats
            'heal_only': healing,
            'healing': healing,
            'healing_per_sec': round(healing / duration_sec, 2) if healing else 0,
            'cleanses': getattr(player, 'cleanses', 0) or 0,
            'cleanses_per_sec': round((getattr(player, 'cleanses', 0) or 0) / duration_sec, 2),
            'resurrects': getattr(player, 'resurrects', 0) or 0,
            'barrier': getattr(player, 'barrier_out', 0) or 0,
            # Boon generation
            'boon_gen': {},
            'boon_uptime': {},
            # Meta
