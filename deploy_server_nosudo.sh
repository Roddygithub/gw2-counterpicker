...
su -c "tee /etc/systemd/system/gw2-counterpicker.service > /dev/null <<'EOF'
[Unit]
...
su -c "systemctl enable gw2-counterpicker"
su -c "systemctl start gw2-counterpicker"

echo "=== Configuration Nginx ==="
# Configurer Nginx comme reverse proxy
su -c "tee /etc/nginx/sites-available/gw2-counterpicker > /dev/null <<'EOF'
server {
    listen 80;
...
su -c "ln -sf /etc/nginx/sites-available/gw2-counterpicker /etc/nginx/sites-enabled/"
su -c "rm -f /etc/nginx/sites-enabled/default"
su -c "nginx -t"
